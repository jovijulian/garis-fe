...
import EditModal from "@/components/modal/edit/EditReminderTypeModal";
...
export default function ReminderTypePage() {
    ...
    const columnsNew = useMemo(() => {
        const defaultColumns = [
            ...
            {
                id: "notification_intervals",
                header: "Jadwal Notifikasi",
                accessorKey: "notification_intervals",
                cell: ({ row }: any) => {
                    let intervals: number[] = [];

                    try {
                        intervals = JSON.parse(row.notification_intervals);
                    } catch (e) { }

                    return (
                        <div className="flex flex-wrap gap-1">
                            {intervals.map((item: number, index: number) => (
                                <span
                                    key={index}
                                    className="px-2 py-1 text-xs rounded bg-blue-100 text-blue-700"
                                >
                                    {item === 0 ? "Hari H" : `H-${item}`}
                                </span>
                            ))}
                        </div>
                    );
    ...
    const getData = async () => {
        ...
        try {
            const response = await httpGet(
                endpointUrl("reminder-types"), true, params
            );
        ...
        <div className="space-y-4">
            ...
            <div className="flex justify-end items-center">
                <div className="flex gap-2">
                    ...
                    <button
                        onClick={() => router.push("/reminders/reminder-types/create")}
            ...
            <DeactiveModal
                ...
                url={`reminder-types/${selectedData?.id}`}
                itemName={selectedData?.name || ""}
                selectedData={selectedData}
                onSuccess={getData}
                message="Jenis Pengingat berhasil dihapus!"
            />

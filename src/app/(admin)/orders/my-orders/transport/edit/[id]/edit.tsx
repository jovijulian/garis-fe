...
import {
    ...
    Users, MapPin, Calendar, Clock, Ticket
} from 'lucide-react';
...
export default function EditTransportPage() {
    ...
    const fetchInitialData = useCallback(async () => {
        ...
        try {
            const [sitesRes, typesRes, detailRes] = await Promise.all([
                httpGet(endpointUrl("/rooms/site-options"), true),
                httpGet(endpointUrl("/transport-types/options"), true),
                httpGet(endpointUrl(`/transport-orders/${id}`), true) 
            ]);
            ...
            setHeaderData({
                ...
                date: data.date,
                time: data.time,
                transport_class: data.transport_class || '',
            ...
            if (data.passengers && data.passengers.length > 0) {
                setPassengers(data.passengers.map((p: any) => ({
                    ...
                    phone_number: p.phone_number ? p.phone_number.toString() : ''
                })));
            }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await httpPut(endpointUrl(`/transport-orders/${id}`), payload, true);

            toast.success("Pesanan transportasi berhasil diperbarui!");
            router.push('/orders/my-orders');
        } catch (error: any) {
            toast.error(error?.response?.data?.message || "Gagal memperbarui pengajuan.");
        } finally {
    ...
    if (isLoadingData) {
        return (
            <ComponentCard title="Ubah Pesanan Transportasi">
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-blue-50 p-4 rounded-xl border border-blue-100">
                        <div className="space-y-3">
                            ...
                            <div>
                                <label className="block font-medium text-xs text-gray-500 mb-1">Detail Lokasi (Stasiun/Bandara/Terminal)</label>
                                <Input
                                    defaultValue={headerData.origin_detail}
                                    onChange={(e) => handleFieldChange('origin_detail', e.target.value)}
                                    placeholder="Contoh: Stasiun Gambir, Pintu Selatan"
                                />
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <Input
                                ...
                                placeholder="Contoh: Garuda Indonesia / Argo Parahyangan"
                            />
                        </div>
                        ...
                        <div className="md:col-span-2">
                            ...
                            <Input
                                ...
                                placeholder="Contoh: Tolong carikan kursi dekat jendela / lorong"
                            />
                        </div>
                    </div>
                </div>

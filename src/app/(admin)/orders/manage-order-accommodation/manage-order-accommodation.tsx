...
import ExportOrderModal from '@/components/modal/ExportOrderAccommodationModal';
...
interface AccommodationCabang {
    id_cab: number;
    nama_cab: string;
}

type AccommodationStatus = "Approved" | "Canceled" | "Pending";
...
export default function ManageOrderPage() {
    ...
    const [selectedData, setSelectedData] = useState<Accommodation | null>(null);
    ...
    const getData = async () => {
        ...
        try {
            const response = await httpGet(endpointUrl("accommodations"), true, params);
    ...
    const columns = useMemo(() => [
        ...
        },
        {
            ...
            accessorKey: "room_neededr",
            accessorFn: (row: any) => row.room_needed,
            cell: ({ row }: any) => <span>{row.room_needed}</span>,
        },
        {
            id: "check_in_date",
            header: "Tanggal Check-In",
            accessorFn: (row: any) => row.check_in_date,
            cell: ({ row }: { row: any }) => (
                <span>{moment(row.check_in_date).format("DD MMM YYYY")}</span>
            ),
        },
        {
            id: "check_out_date",
            header: "Tanggal Check-Out",
            accessorFn: (row: any) => row.check_out_date,
            cell: ({ row }: { row: any }) => (
                <span>{moment(row.check_out_date).format("DD MMM YYYY")}</span>
            ),
        },
        {
            id: "total_pax",
            ...
            accessorFn: (row: any) => row.total_pax,
            cell: ({ row }: any) => <span>{row.total_pax}</span>,
        },
        {
            id: "status",
            header: "Status",
    ...
    const handleOpenModal = (booking: Accommodation, action: 'Approved' | 'Rejected') => {
    ...
    const handleUpdateStatus = async () => {
        ...
        try {
            await httpPut(endpointUrl(`accommodations/status/${selectedData.id}`), { status: actionType }, true);
    ...
    const handleOpenCancelModal = (booking: Accommodation) => {
        setSelectedData(booking);
    ...
    const handleConfirmCancel = async () => {
        ...
        setIsSubmitting(true);
        try {
            await httpPut(endpointUrl(`accommodations/cancel/${selectedData.id}`), {}, true);
    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-end items-center gap-2">
                ...
                <button
                    onClick={() => router.push("/orders/create-accommodation-admin")}

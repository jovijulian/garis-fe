...
export default function CreateOrderAdminPage() {
    ...
    const [userOptions, setUserOptions] = useState<SelectOption[]>([]); // <-- State baru untuk daftar user
    ...
    useEffect(() => {
        ...
        const fetchInitialData = async () => {
            try {
                const [bookingsRes, roomsRes, sitesRes, consumptionTypesRes, usersRes] = await Promise.all([
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            router.push('/orders/manage-order');
        } catch (error: any) {
    ...
    return (
        <ComponentCard title="Buat Pesanan Baru (Admin)">
            <form onSubmit={handleSubmit} className="space-y-6">
                {/* ... (Pilihan Tipe Lokasi tidak berubah) ... */}
                ...
                {locationType === 'room' && (
                    <div className="space-y-4">
                        <div>
                            ...
                            <Select options={userOptions} value={_.find(userOptions, { value: formData.user_id })} onValueChange={(opt) => handleFieldChange('user_id', opt ? opt.value : null)} placeholder="Pilih nama pengguna..." />
                        </div>
                ...
                {locationType === 'custom' && (
                    <div className="space-y-4">
                        {/* --- TAMBAHKAN PEMILIHAN USER DI SINI JUGA --- */}
                        <div>
                            <label className="block font-medium mb-1">Pilih Pengguna (Pemesan)</label>
                            <Select options={userOptions} value={_.find(userOptions, { value: formData.user_id })} onValueChange={(opt) => handleFieldChange('user_id', opt ? opt.value : null)} placeholder="Pilih nama pengguna..." />
                        </div>

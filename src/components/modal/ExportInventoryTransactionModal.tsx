...
const ExportInventoryTransactionModal: React.FC<ExportModalProps> = ({ isOpen, onClose }) => {
    ...
    const handleDownload = async () => {
        ...
        if (status) {
            params.append('transaction_type', status);
        }

        try {
            const response = await fetch(endpointUrl(`/inventory-transactions/export-excel?${params.toString()}`), {
                headers: {
            ...
            a.href = url;
            a.download = `inventory_transaction_report_${moment().format('YYYY-MM-DD')}.xlsx`;
            ...
            <div className="p-6">
                ...
                <div className="space-y-4 grid">
                    <div>
                     ...
                     <div>
                        <label className="block font-medium mb-2">Filter Berdasarkan Tipe Transaksi</label>
                        <Select
                            options={transactionType}
                            onValueChange={(opt) => setStatus(opt ? opt.value : '')}
                            value={_.find(transactionType, { value: status })}
                        />
                    </div>
                </div>
...
export default ExportInventoryTransactionModal;
...
const EditConsumptionTypeModal: React.FC<EditProps> = ({
    isOpen,
    selectedId,
    onClose,
    onSuccess,
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setError("");
            try {
                const response = await httpGet(endpointUrl(`consumption-types/${selectedId}`), true);
                ...
                toast.error(err?.response?.data?.message || "Failed to fetch consumption type data.");
                setError("Could not load consumption type data.");
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleSubmit = async () => {
        ...
        try {
            await httpPut(endpointUrl(`consumption-types/${selectedId}`), payload, true);
            toast.success("Berhasil mengubah tipe konsumsi");
            setName("");
            setDescription("");
            onClose();
            onSuccess?.();
            ...
            setError(error?.response?.data?.message || "Gagal mengubah tipe konsumsi");
        }
            ...
            <div className="no-scrollbar relative w-full max-w-[500px] overflow-y-auto rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-8">
                ...
                >
                    <div className="space-y-5 px-2 pb-3">
                        <div>
                            <Label htmlFor="name">Nama tipe konsumsi</Label>
                            <Input
                                type="text"
                                id="name"
                                name="name"
...
export default EditConsumptionTypeModal;

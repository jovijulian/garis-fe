...
const EditInventoryCategoryModal: React.FC<EditProps> = ({
    isOpen,
    selectedId,
    onClose,
    onSuccess,
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setError("");
            try {
                const response = await httpGet(endpointUrl(`inventory-categories/${selectedId}`), true);
                const data = response.data.data;

                setName(data.name || "");

            } catch (err: any) {
                toast.error(err?.response?.data?.message || "Failed to fetch category data.");
    ...
    const handleSubmit = async () => {
        ...
        try {
            await httpPut(endpointUrl(`inventory-categories/${selectedId}`), payload, true);
            ...
            setError(error?.response?.data?.message || "Gagal mengubah kategori");
        }

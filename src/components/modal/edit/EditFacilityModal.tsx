...
const EditRoomModal: React.FC<EditProps> = ({
    ...
    useEffect(() => {
        const fetchRoomData = async () => {
            ...
            setError("");
            try {
                const response = await httpGet(endpointUrl(`amenities/${selectedId}`), true);
    ...
    const handleSubmit = async () => {
        ...
        try {
            await httpPut(endpointUrl(`amenities/${selectedId}`), payload, true);
            toast.success("Berhasil mengubah fasilitas");
            setName("");
            setDescription("");
            onClose();
            onSuccess?.();
            ...
            setError(error?.response?.data?.message || "Gagal mengubah fasilitas");
        }

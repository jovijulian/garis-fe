...
const EditInventoryUnitModal: React.FC<EditProps> = ({
    isOpen,
    selectedId,
    onClose,
    onSuccess,
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setError("");
            try {
                const response = await httpGet(endpointUrl(`inventory-units/${selectedId}`), true);
                const data = response.data.data;

                setName(data.name || "");

            } catch (err: any) {
                toast.error(err?.response?.data?.message || "Failed to fetch unit data.");
    ...
    const handleSubmit = async () => {
        ...
        try {
            await httpPut(endpointUrl(`inventory-units/${selectedId}`), payload, true);
            toast.success("Berhasil mengubah unit");
            setName("");
            onClose();
            onSuccess?.();
        } catch (error: any) {
            toast.error(error?.response?.data?.message);
            setError(error?.response?.data?.message || "Gagal mengubah unit");
        }
...
export default EditInventoryUnitModal;

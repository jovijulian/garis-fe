...
const EditTransportTypeModal: React.FC<EditProps> = ({
    isOpen,
    selectedId,
    onClose,
    onSuccess,
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setError("");
            try {
                const response = await httpGet(endpointUrl(`transport-types/${selectedId}`), true);
                const transportData = response.data.data;

                setName(transportData.name || "");

            } catch (err: any) {
                toast.error(err?.response?.data?.message || "Failed to fetch transport type data.");
                setError("Could not load transport type data.");
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleSubmit = async () => {
        ...
        try {
            await httpPut(endpointUrl(`transport-types/${selectedId}`), payload, true);
            toast.success("Berhasil mengubah jenis transportasi");
            setName("");
            onClose();
            onSuccess?.();
        } catch (error: any) {
            toast.error(error?.response?.data?.message);
            setError(error?.response?.data?.message || "Gagal mengubah jenis transport");
        }
            ...
            <div className="no-scrollbar relative w-full max-w-[500px] overflow-y-auto rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-8">
                ...
                >
                    <div className="space-y-5 px-2 pb-3">
                        <div>
                            <Label htmlFor="name">Nama jenis transportasi</Label>
                            <Input
                                type="text"
                                id="name"
                                name="name"
